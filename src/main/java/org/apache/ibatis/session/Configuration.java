...
public class Configuration {
  ...
  /**
   * StrictMap 重写了 put get 方法
   *
   * 将id和namespace+id的键，都put了进去，指向同一个MappedStatement对象
   *
  ...
  //s 解析<cache-ref> 没有成功的  CacheRefResolver
   ...
   */
  //s 缓存映射 key <cache-ref> 节点所在的 namespace value <cache-ref> 节点的 namespace
  ...
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    ...
    //s 开启二级缓存 使用 CachingExecutor 装饰
    if (cacheEnabled) {
      executor = new CachingExecutor(executor);
    }
    //s 通过 InterceptorChain.pluginAll （）方法创建 Executor 的代理对象
